...
import { numberPaginationFun, toggleAction, toggleActionWhenwithId } from "@/redux/reducer";
    ...
    <div>
        ...
        <tbody>
          ...
          {data?.data?.map((user, i) => (
            <tr className="h-10" key={i}>
              <td onClick={() => router.push(`${user.id}`)} className="p-2  border">{user?.name}</td>
      ...
      <div className="">
        <ul className={`flex justify-around items-center m-auto bg-slate-100 mt-5 w-1/2 h-10 rounded-full ${isFetching && "border-black"} border relative duration-500`}>
          ...
          <li>
            <button onClick={() => handleAddPagePaginationToReduxNext()} disabled={data.data.length < 5 || data.data.length == 4 }  className={`flex items-center top-0 ${data.data.length < 5 ? "scale-100 bg-slate-400 absolute right-0 " : "scale-110 bg-slate-200 right-20"} py-2 px-4 rounded-full scale-110 text-gray-900 hover:scale-100 duration-500`}>
              <span className="mr-1">
                Next
              </span>
              <span>
